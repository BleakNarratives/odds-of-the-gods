...
const Torch: React.FC<{ children?: React.ReactNode; isEliminated?: boolean }> = ({ children, isEliminated }) => (
    <div className={`relative w-full h-full transition-opacity ${isEliminated ? 'opacity-30' : ''}`}>
        {children}
        ...
        {isEliminated && <div className="absolute inset-0 flex items-center justify-center"><div className="w-16 h-1 bg-red-500 transform rotate-45"></div><div className="w-16 h-1 bg-red-500 transform -rotate-45"></div></div>}
    </div>
);

// Component for the choice phase
export const HecateCrossroadsChoice: React.FC<HecateCrossroadsChoiceProps> = ({ onChoice, eliminatedTorch }) => {
    return (
            ...
            <div className="flex gap-8 md:gap-20">
                {[0, 1, 2].map(i => (
                    <button
                        key={i}
                        onClick={() => onChoice(i)}
                        className="w-24 h-24 relative group disabled:cursor-not-allowed"
                        aria-label={`Choose torch ${i + 1}`}
                        disabled={eliminatedTorch === i}
                    >
                         <Torch isEliminated={eliminatedTorch === i} />
                    </button>
                ))}
            </div>
        </div>
...
const TorchWithFlame: React.FC<{ index: number; animation: string; isWinning: boolean; isRevealed: boolean; isEliminated?: boolean; }> = ({ index, animation, isWinning, isRevealed, isEliminated }) => (
    <div
    ...
    >
        <div className="relative w-full h-full">
            ...
            <Torch isEliminated={isEliminated}/>
        </div>
    </div>
);
...
export const HecateCrossroadsAnimation: React.FC<HecateCrossroadsAnimationProps> = ({ isWin, choice, onAnimationEnd, eliminatedTorch }) => {
    ...
    const winningTorchIndex = useMemo(() => {
        ...
        let otherTorches = [0, 1, 2].filter(i => i !== choice && i !== eliminatedTorch);
        return otherTorches[0];
    }, [isWin, choice, eliminatedTorch]);
             ...
             <div className="relative w-full h-48">
                {[0, 1, 2].map(i => (
                    <TorchWithFlame 
                        ...
                        isEliminated={i === eliminatedTorch}
                    />
                ))}
            </div>
