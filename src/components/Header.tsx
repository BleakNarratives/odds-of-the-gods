...
import { ThothIcon, SoulIcon, ArchitectIcon, DevToolsIcon, SurveyIcon } from './icons/MythicIcons';
...
const Header: React.FC<HeaderProps> = ({ souls, onCashOutClick, onReplenishClick, showMenuButton, onMenuClick, onThothClick, onQuestsClick, unclaimedQuestsCount, soulUpdateClass, onArchitectClick, onDevToolsClick, onSurveyClick, isSurveyAvailable, onRaveToggle }) => {
  const [tapCount, setTapCount] = useState(0);
  const tapTimeout = useRef<number | null>(null);

  const handleThothTap = () => {
    ...
    tapTimeout.current = window.setTimeout(() => {
      setTapCount(0);
    }, 1500); // Reset tap count after 1.5s
  };
    ...
    <header className="bg-black/20 backdrop-blur-sm border-b border-theme-border/50 sticky top-0 z-50">
      <div className="container mx-auto px-4 md:px-8 py-4 flex justify-between items-center">
        ...
        <div className="flex items-center gap-3">
          <button onClick={onThothClick} onTouchStart={handleThothTap} className="group relative" aria-label="Seek Thoth's Counsel">
          ...
          {isSurveyAvailable && (
            <button onClick={onSurveyClick} className="group relative" aria-label="Provide Feedback">
              <SurveyIcon className="w-7 h-7 text-cyan-400 transition-all duration-300 group-hover:text-white group-hover:scale-110" />
              <div className="absolute top-0 right-0 w-2 h-2 rounded-full bg-cyan-400 ring-2 ring-slate-800"></div>
            </button>
          )}
