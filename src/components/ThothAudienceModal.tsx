// src/components/ThothAudienceModal.tsx
...
export const ThothAudienceModal: React.FC<ThothAudienceModalProps> = ({ isOpen, onClose, onApplyBoon, playerState, souls }) => {
    ...
    if (isOpen) {
      ...
      const startSession = async () => {
        ...
        const draw = () => {
            ...
            canvasCtx.fillStyle = 'rgba(29, 27, 49, 0.1)'; // Faded background clear
  ...
  const getStatusIndicator = () => {
    switch(status) {
        ...
        case 'LISTENING': return { text: isMuted ? "Muted" : "Speak, mortal...", color: isMuted ? 'border-slate-500' : 'border-green-500 animate-pulse' };
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-lg flex items-center justify-center z-[100] animate-fade-in" onClick={handleClose}>
      <div className="bg-slate-900/50 rounded-lg shadow-2xl shadow-theme-primary/20 border border-theme-border w-full max-w-2xl m-4 p-8 flex flex-col items-center text-center h-[80vh]" onClick={e => e.stopPropagation()}>
        ...
        <button
          onClick={toggleMute}
          className={`my-4 px-4 py-2 rounded-lg font-bold transition-colors ${
            isMuted
        ...
        <div className="w-full h-12 mb-4">
            <canvas ref={visualizerRef} width="600" height="80" className="w-full h-full"></canvas>
        </div>
