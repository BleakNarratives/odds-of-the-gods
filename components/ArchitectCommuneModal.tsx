...
const ArchitectCommuneModal: React.FC<ArchitectCommuneModalProps> = ({ isOpen, onClose }) => {
                        ...
                        {messages.map((msg, index) => (
                            <div key={index} className={`flex items-start gap-2.5 ${msg.role === 'user' ? 'justify-end' : ''}`}>
                                ...
                                <div className={`p-3 rounded-lg max-w-xs md:max-w-md ${
                                    msg.role === 'user' ? 'bg-theme-primary/80 text-theme-background' : 
                                    msg.role === 'model' ? 'bg-slate-700 text-slate-200' : 
