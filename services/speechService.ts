...
class SpeechService {
  ...
  private loadVoices() {
    ...
    this.thothVoice = this.voices.find(voice => voice.name === THOTH_VOICE_NAME) || 
                      this.voices.find(voice => voice.lang.startsWith('en-GB') && voice.name.includes('Male')) ||
  ...
  public speak(text: string, rate = 0.9, pitch = 0.8) {
    ...
    // Cancel any currently speaking utterance to prioritize the new one.
    if (this.synth.speaking) {
      this.synth.cancel();
      this.isSpeaking = false; // Force state reset
      this.messageQueue = []; // Clear the queue as well
    }
